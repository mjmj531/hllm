...
def bulid_dataloader(config, dataload):
    ...
    dataload.build() # 调用dataload.py中的build()函数，生成训练集、验证集、测试集
    ...
    if isinstance(train_set_name, tuple):
        ...
        train_collate_fn = getattr(dataset_module, train_set_name[1]) # REC.data.dataset.customize_rmpad_collate
    else:
    ...
    if train_collate_fn: # HLLM默认有
        train_loader = DataLoader(train_data, batch_size=config['train_batch_size'], num_workers=num_workers,
                                  pin_memory=True, sampler=train_sampler, collate_fn=train_collate_fn, worker_init_fn=init_fn) # collate_fn是合成样本列表形成batch的函数
    else:
        train_loader = DataLoader(train_data, batch_size=config['train_batch_size'], num_workers=num_workers,
    ...
    test_loader = DataLoader(test_data, batch_size=config['eval_batch_size'], num_workers=num_workers,
                             pin_memory=True, sampler=test_sampler, collate_fn=eval_collate_fn) # collate_fn: 指定如何将样本列表合成一个 batch 式的张量或其他数据结构。
...
# 分布式训练环境中对验证和测试数据集进行分区，确保每个进程加载的数据子集是不同的。
# 这个采样器采用的是非连续顺序采样方式，这意味着每个进程不会加载连续的数据，而是间隔地从数据集中采样数据。
# 适用于validation和test数据集，因为它们的顺序是不重要的。
