...
def seq_eval_collate(batch):
    ...
    for item in batch: # item来自evalset中的__getitem__方法
        history_i.append(item[0])
        item_seq.append(item[1])
        item_target.append(item[2])
        time_seq.append(item[3])
    
    # history_u 是一个 1D Tensor，长度等于所有用户交互 item 总数，表示每个交互对应的用户 ID。
    history_u = torch.cat([torch.full_like(hist_iid, i) for i, hist_iid in enumerate(history_i)]) 
    # torch.full_like(hist_iid, i)：对于history_i中的每个用户交互历史hist_iid，生成一个与hist_iid形状相同的新张量，但全部填充为当前用户的索引i。
...
def customize_rmpad_collate(batch):
    r"""Puts each data field into a tensor with outer dimension batch size"""
    # 用于将批量数据（batch）中的每个字段转换为张量，并确保外部维度是批量大小。
    # 它的主要目的是处理不同类型的数据（如张量、NumPy 数组、标量、字典等），并将它们以适当的方式组合成批量张量。
