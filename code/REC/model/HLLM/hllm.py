...
class HLLM(BaseModel):
    ...
    def __init__(self, config, dataload):
        ...
        self.use_ft_flash_attn = config['use_ft_flash_attn'] # true

        # 创建项目和用户语言模型
        self.logger.info(f"create item llm")
        self.item_llm = self.create_llm(self.item_pretrain_dir, config['item_llm_init']) # init=true
        self.logger.info(f"create user llm")
        self.user_llm = self.create_llm(self.user_pretrain_dir, config['user_llm_init'])

        # 项目嵌入令牌数量
        self.item_emb_token_n = config['item_emb_token_n'] # 此处固定为1
    ...
    def nce_loss(self, cur_embs, target_pos, target_neg, user_attention_mask):
        ...
        logit_scale = self.logit_scale.exp() # exp(logit_scale)
        D = target_neg.size(-1)
    ...
    def forward_item_emb(
        ...
        input_ids,
        position_ids,
        cu_input_lens, # item序列的累计长度，用于确定每个序列的结束位置
        emb_token_n, # 1
        emb_tokens, # shape [1, 1, hidden_size]
        llm
    ):
        # 计算项目嵌入
        inputs_embeds = llm.get_input_embeddings()(input_ids)
        emb_pos = cu_input_lens.cumsum(dim=0, dtype=torch.int32) # 确定嵌入位置
        if emb_token_n > 0:
            inputs_embeds[emb_pos - 1] = emb_tokens # 如果emb_token_n大于0，则将每个序列的最后一个位置的嵌入向量替换为emb_tokens。这旨在在序列的末尾插入一个特定的项目嵌入令牌。
        model_out = llm(inputs_embeds=inputs_embeds.unsqueeze(0), cu_input_lens=cu_input_lens, position_ids=position_ids.unsqueeze(0)) # (total_sequence_length, hidden_size)
        model_out = model_out.hidden_states[-1].squeeze(0) 

        if emb_token_n > 0:
            emb = model_out[emb_pos - 1] # 如果emb_token_n大于0，则提取每个序列最后一个位置的隐藏状态作为项目嵌入
        else: # 计算序列均值
    ...
    def forward(self, interaction, mode='train'): # 选取model，共有三种，是在trainer.py中调用的
        # 前向传播
        if mode == 'predict':
        ...
        pos_embedding = pos_embedding.reshape(N, S+1, -1) # （N， S+1， hidden_size）
        ...
        target_pos_embs = pos_embedding[:, 1:] # target_pos_embs.shape == (N, S, hidden_size)
    ...
    @torch.no_grad()
    def predict(self, item_seq, time_seq, item_feature): # 三个参数分别对应interaction[0], interaction[1], interaction[2]
        attention_mask = (item_seq > 0).int()

        pos_embedding = item_feature[item_seq]

        user_embedding = self.user_llm(inputs_embeds=pos_embedding, attention_mask=attention_mask).hidden_states[-1] # 获取用户语言模型的最后一个隐藏层的输出状态，这个状态代表了用户对交互序列中所有物品的综合特征表示。
        seq_output = user_embedding[:, -1] # 从 user_embedding 中提取最后一个时间步的隐藏状态
        seq_output = seq_output / seq_output.norm(dim=-1, keepdim=True)
        item_feature = item_feature / item_feature.norm(dim=-1, keepdim=True)

        # 由于 seq_output 和 item_feature 都已经进行了L2归一化，
        # 矩阵乘法的结果实际上是用户嵌入向量与每个物品特征向量之间的余弦相似度。
        return torch.matmul(seq_output, item_feature.t()) # 矩阵乘法
