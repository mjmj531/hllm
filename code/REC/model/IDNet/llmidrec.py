...
# LLMIDRec是把用户点击的物品ID序列，直接embedding后喂给大语言模型（Llama或BERT），用LLM来理解用户兴趣变化，最后通过对比学习NCE loss来训练推荐系统！
# 物品ID ➔ item_embedding ➔ 输入到LLM ➔ 用户兴趣向量 ➔ 检索打分 ➔ TopK推荐
...
class LLMIDRec(BaseModel):
    ...
    def __init__(self, config, dataload):
        ...
        # 如果物品embedding维度 ≠ LLM hidden size，需要加一层线性映射（item_id_proj_tower）统一维度。
    ...
    def forward(self, interaction):
        ...
        output_embs = self.user_llm(inputs_embeds=input_emb, attention_mask=masked_index).hidden_states[-1] # 拿最后一层hidden state作为输出
