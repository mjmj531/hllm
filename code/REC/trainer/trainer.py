...
class Trainer(object):
    def __init__(self, config, model):
        ...
        self.eval_step = min(config['eval_step'], self.epochs) # config中为1，指每隔多少个epoch进行一次evaluate
        self.stopping_step = config['stopping_step']
        self.clip_grad_norm = config.get('clip_grad_norm', 1.0)
        self.valid_metric = config['valid_metric'].lower() # valid_metric: NDCG@200 # 指定评估指标
        ...
        self.use_text = config['use_text'] #默认为true
    ...
    def _valid_epoch(self, valid_data, show_progress=False):
        torch.distributed.barrier() # 是 PyTorch 分布式训练中的一个同步操作，用于在所有参与分布式训练的进程之间设置一个屏障（Barrier）。当某个进程执行到 barrier() 时，它会等待，直到所有其他进程也到达这个屏障位置，然后所有进程才会继续执行后续代码。)
    ...
    def fit(self, train_data, valid_data=None, verbose=True, saved=True, show_progress=False, callback_fn=None):
        ...
        precision = self.config['precision'] if self.config['precision'] else '32' # 精度
        if self.config['strategy'] == 'deepspeed': # Deepseed 是深度学习优化库，旨在加速大规模分布式训练，特别是在使用多 GPU 和多节点的情况下。以下是代码的详细解释：
        ...
        # 主训练循环
        for epoch_idx in range(self.start_epoch, self.epochs): # HLLMtrain了5个epoch
            if self.config['need_training'] == None or self.config['need_training']: # 'need training'意义不明
    ...
    def _full_sort_batch_eval(self, batched_data):
        # item_seq, time_seq, (history_u, history_i), positive_u, item_target
        user, time_seq, history_index, positive_u, positive_i = batched_data # history是用户的历史交互记录索引，来自collect_fn.py
        ...
        if self.config['model'] == 'HLLM':
            if self.config['stage'] == 3:
                ...
                scores = self.model((interaction, time_seq, self.item_feature), mode='predict') # HLLM，直接调用 predict 方法
        else:
            scores = self.model.module.predict(interaction, time_seq, self.item_feature)
        scores = scores.view(-1, self.tot_item_num) #shape: (batch_size, tot_item_num)
        scores[:, 0] = -np.inf
        if history_index is not None:
            scores[history_index] = -np.inf # 将历史交互的分数置为负无穷，防止在推荐时重复推荐用户已经交互过的项目。
        return scores, positive_u, positive_i

    # 计算所有item的特征表示
    @torch.no_grad()
    def compute_item_feature(self, config, data): # 在evaluate方法中调用，计算所有item的特征表示
        if self.use_text:
            item_data = BatchTextDataset(config, data)
            item_batch_size = config['MAX_ITEM_LIST_LENGTH'] * config['train_batch_size'] ######################## inference调用
            item_loader = DataLoader(item_data, batch_size=item_batch_size, num_workers=14, shuffle=False, pin_memory=True, collate_fn=customize_rmpad_collate) # 多线程，顺序加载，加速GPU数据传输，自定义批次处理函数
    ...
    def evaluate(self, eval_data, load_best_model=True, model_file=None, show_progress=False, init_model=False):
        ...
        if init_model:
            ...
            if self.config['strategy'] == 'deepspeed':
                # 如果使用的是 deepspeed 策略，则配置并初始化相应的策略
                self.logger.info(f"Use deepspeed strategy")
                precision = self.config['precision'] if self.config['precision'] else '32' # 指定精度
                strategy = DeepSpeedStrategy(stage=self.config['stage'], precision=precision)
                self.lite = L.Fabric(accelerator='gpu', strategy=strategy, precision=precision, num_nodes=nnodes)
                self.lite.launch()
                self.model, self.optimizer = self.lite.setup(self.model, self.optimizer)
            else:
                # 如果使用的是 DDP 策略，则配置并初始化相应的策略
                self.logger.info(f"Use DDP strategy") # DDP： 分布式数据并行
        ...
        with torch.no_grad():
            ...
            metric_decimal_place = 5 if self.config['metric_decimal_place'] == None else self.config['metric_decimal_place'] #yaml中为7,保留的小数位数
            # 根据配置的精度对评估结果进行四舍五入，并进行分布式拼接
